...
public class EventCommandController {
    ...
    @Operation(summary = "알림 이벤트 읽음 처리", description = "알림 이벤트를 읽음 처리합니다. status는 변경되지 않고 acknowledgedAt만 설정됩니다. 사용자 ID는 X-User-ID 헤더에서 자동으로 추출됩니다.")
    @PostMapping("/{id}/acknowledge")
    public ApiResponse<EventResponse> acknowledge(@PathVariable Long id) {
        Long memberId = UserIdInterceptor.getCurrentUserId();
        return ApiResponse.ok(200, eventCommandService.acknowledge(id, memberId), 
            "이벤트가 읽음 처리되었습니다.");
    }
    ...
    @Operation(summary = "알림 이벤트 해결", description = "알림 이벤트를 해결 처리합니다. 사용자 ID는 X-User-ID 헤더에서 자동으로 추출됩니다.")
    @PostMapping("/{id}/resolve")
    public ApiResponse<EventResponse> resolve(@PathVariable Long id) {
        Long memberId = UserIdInterceptor.getCurrentUserId();
        return ApiResponse.ok(200, eventCommandService.resolve(id, memberId), 
            "이벤트가 해결되었습니다.");
    }

    @Operation(summary = "알림 이벤트 처리 이력 추가", description = "알림 이벤트에 처리 이력을 추가합니다. 처리내역 작성 시 자동으로 status가 CLOSED로 변경됩니다. 사용자 ID는 X-User-ID 헤더에서 자동으로 추출됩니다.")
    @PostMapping("/{id}/history")
    public ApiResponse<ProgressHistoryResponse> addHistory(
            @PathVariable Long id,
            @Valid @RequestBody ProgressHistoryCreateRequest request) {
        Long memberId = UserIdInterceptor.getCurrentUserId();
        return ApiResponse.ok(200, eventCommandService.addHistory(id, memberId, request.getContent()), 
